/**
 * ScholarHub — School MIS (MongoDB Atlas/Render Version)
 */
...
async function connectDB() {
  try {
    ...
    console.error('Please check your MONGO_URL string in the code.');
  }
}
...
async function handleAPI(req, res) {
  ...
  if (url === '/api/login' && method === 'POST') {
    ...
    const token = makeToken({ role: 'student', id: st._id });
    res.writeHead(200, { 'Set-Cookie': `sh_token=${token}; Path=/; HttpOnly`, 'Content-Type': 'application/json' });
  ...
  if (url === '/api/register' && method === 'POST') {
    ...
    try {
      await db.collection('students').insertOne({ 
        id: Date.now(), name, roll, cls, section, username, password: hashPass(password), approved: false, createdAt: new Date().toISOString() 
      });
      json(res, 200, { ok: true, msg: 'Registration successful! Awaiting admin approval.' });
    } catch (e) {
      if (e.code === 11000) return json(res, 400, { ok: false, msg: 'Username already taken.' });
      json(res, 500, { ok: false, msg: 'Error registering.' });
    }
    return;
  }
  ...
  if (url === '/api/me' && method === 'GET') {
    ...
    const st = await db.collection('students').findOne({ _id: new ObjectId(user.id) });
  ...
  if (url === '/api/students' && method === 'GET') {
    ...
    json(res, 200, { ok: true, students: students.map(sanitizeStudent) });
    return;
  }

  // POST /api/students/:id/approve
  const approveMatch = url.match(/^\/api\/students\/([a-f0-9]+)\/approve$/);
  if (approveMatch && method === 'POST') {
    if (user.role !== 'admin') return json(res, 403, {});
    await db.collection('students').updateOne({ _id: new ObjectId(approveMatch[1]) }, { $set: { approved: true } });
    json(res, 200, { ok: true });
    return;
  }

  // DELETE /api/students/:id
  const delStudentMatch = url.match(/^\/api\/students\/([a-f0-9]+)$/);
  if (delStudentMatch && method === 'DELETE') {
    if (user.role !== 'admin') return json(res, 403, {});
    const id = delStudentMatch[1];
    await db.collection('students').deleteOne({ _id: new ObjectId(id) });
    await db.collection('grades').deleteMany({ sid: id });
    await db.collection('attendance').deleteMany({ sid: id });
    json(res, 200, { ok: true });
    return;
  }
  ...
  if (url === '/api/grades' && method === 'POST') {
    ...
    if (parseInt(obt) > parseInt(max)) return json(res, 400, { ok: false, msg: 'Obtained cannot exceed max marks.' });
    await db.collection('grades').insertOne({ id: Date.now(), sid, sub, exam, max: parseInt(max), obt: parseInt(obt), createdAt: new Date().toISOString() });
    json(res, 200, { ok: true });
    return;
  }

  // DELETE /api/grades/:id
  const delGradeMatch = url.match(/^\/api\/grades\/([a-f0-9]+)$/);
  if (delGradeMatch && method === 'DELETE') {
    if (user.role !== 'admin') return json(res, 403, {});
    await db.collection('grades').deleteOne({ _id: new ObjectId(delGradeMatch[1]) });
    json(res, 200, { ok: true });
    return;
  }
  ...
  if (url === '/api/attendance' && method === 'GET') {
    const sid = user.role === 'student' ? user.id : null;
    const query = sid ? { sid: sid } : {};
    const att = await db.collection('attendance').find(query).toArray();
  ...
  if (url === '/api/attendance' && method === 'POST') {
    ...
    await db.collection('attendance').insertOne({ id: Date.now(), sid, sub, tot: parseInt(tot), pre: parseInt(pre), createdAt: new Date().toISOString() });
    json(res, 200, { ok: true });
    return;
  }

  // DELETE /api/attendance/:id
  const delAttMatch = url.match(/^\/api\/attendance\/([a-f0-9]+)$/);
  if (delAttMatch && method === 'DELETE') {
    if (user.role !== 'admin') return json(res, 403, {});
    await db.collection('attendance').deleteOne({ _id: new ObjectId(delAttMatch[1]) });
    json(res, 200, { ok: true });
    return;
  }
  ...
  if (url === '/api/homework' && method === 'GET') {
    let hw = await db.collection('homework').find({}).toArray();
  ...
  if (url === '/api/homework' && method === 'POST') {
    ...
    await db.collection('homework').insertOne({ id: Date.now(), title, sub, cls, due, pri, desc, createdAt: new Date().toISOString() });
    json(res, 200, { ok: true });
    return;
  }

  // DELETE /api/homework/:id
  const delHWMatch = url.match(/^\/api\/homework\/([a-f0-9]+)$/);
  if (delHWMatch && method === 'DELETE') {
    if (user.role !== 'admin') return json(res, 403, {});
    await db.collection('homework').deleteOne({ _id: new ObjectId(delHWMatch[1]) });
    json(res, 200, { ok: true });
    return;
  }
  ...
  if (url === '/api/notices' && method === 'POST') {
    ...
    await db.collection('notices').insertOne({ id: Date.now(), title, cat, msg, date, createdAt: new Date().toISOString() });
    json(res, 200, { ok: true });
    return;
  }

  // DELETE /api/notices/:id
  const delNotMatch = url.match(/^\/api\/notices\/([a-f0-9]+)$/);
  if (delNotMatch && method === 'DELETE') {
    if (user.role !== 'admin') return json(res, 403, {});
    await db.collection('notices').deleteOne({ _id: new ObjectId(delNotMatch[1]) });
    json(res, 200, { ok: true });
    return;
  }
  ...
  if (url === '/api/timetable' && method === 'GET') {
    let tt = await db.collection('timetable').find({}).toArray();
    if (user.role === 'student') {
      const st = await db.collection('students').findOne({ _id: new ObjectId(user.id) });
  ...
  if (url === '/api/timetable' && method === 'POST') {
    ...
    await db.collection('timetable').insertOne({ id: Date.now(), cls, day, sub, period });
    json(res, 200, { ok: true });
    return;
  }

  // DELETE /api/timetable/:id
  const delTTMatch = url.match(/^\/api\/timetable\/([a-f0-9]+)$/);
  if (delTTMatch && method === 'DELETE') {
    if (user.role !== 'admin') return json(res, 403, {});
    await db.collection('timetable').deleteOne({ _id: new ObjectId(delTTMatch[1]) });
    json(res, 200, { ok: true });
    return;
  }
  ...
  if (url === '/api/stats' && method === 'GET') {
    if (user.role !== 'admin') return json(res, 403, {});
    const approved = await db.collection('students').countDocuments({ approved: true });
    const pending  = await db.collection('students').countDocuments({ approved: false });
    const hwCount = await db.collection('homework').countDocuments();
    const notCount = await db.collection('notices').countDocuments();
    
    // Calculate class counts
    const classCounts = {};
    const classes = ['Class 6','Class 7','Class 8','Class 9','Class 10','Class 11','Class 12'];
    for(const c of classes) {
      classCounts[c] = await db.collection('students').countDocuments({ cls: c, approved: true });
    }

    json(res, 200, { ok: true, totalStudents: approved, pending: pending, homework: hwCount, notices: notCount, classCounts });
    return;
  }
...
// ── HTML Frontend (Keeping your original UI) ─────────────────────────────────────────────
    ...
    <div id="ap-grades" class="page">
      ...
      <div class="form-section">
        ...
        <div class="f-row f-row-3">
          <div class="f-grp"><label class="f-lbl">Student</label><select class="fs" id="gf-st"><option value="">Select student</option></select></div>
          <div class="f-grp"><label class="f-lbl">Subject</label><input class="fi" id="gf-sub" placeholder="Subject"/></div>
      ...
      <div class="card"><div class="card-header"><div class="card-title">All Grade Records</div></div><div class="tbl-wrap"><table class="tbl"><thead><tr><th>Student</th><th>Subject</th><th>Exam</th><th>Max</th><th>Obtained</th><th>%</th><th>Grade</th><th></th></tr></thead><tbody id="grd-tbody"></tbody></table></div></div>
    </div>
    <div id="ap-attendance" class="page">
      ...
      <div class="form-section">
        ...
        <div class="f-row f-row-4">
          <div class="f-grp"><label class="f-lbl">Student</label><select class="fs" id="af-st"><option value="">Select</option></select></div>
          <div class="f-grp"><label class="f-lbl">Subject</label><input class="fi" id="af-sub" placeholder="Subject"/></div>
      ...
      <div class="card"><div class="card-header"><div class="card-title">Attendance Records</div></div><div class="tbl-wrap"><table class="tbl"><thead><tr><th>Student</th><th>Subject</th><th>Total</th><th>Present</th><th>Absent</th><th>%</th><th>Status</th><th></th></tr></thead><tbody id="att-tbody"></tbody></table></div></div>
    </div>
    <div id="ap-assigns" class="page">
      ...
      <div class="form-section">
        ...
        <div class="f-row f-row-3">
          ...
          <div class="f-grp"><label class="f-lbl">Subject</label><input class="fi" id="hf-sub" placeholder="Subject"/></div>
    ...
    <div id="ap-timetable" class="page">
      ...
      <div class="form-section">
        ...
        <div class="f-row f-row-4">
          ...
          <div class="f-grp"><label class="f-lbl">Subject</label><input class="fi" id="tf-sub" placeholder="Subject"/></div>
          <div class="f-grp"><label class="f-lbl">Period</label><input class="fi" id="tf-period" placeholder="Period Time"/></div>
        </div>
...
async function loadGradesPage() {
  ...
  document.getElementById('gf-st').innerHTML = '<option value="">Select student</option>' +
    allStudents.map(s => \`<option value="\${s._id}">\${s.name} (\${s.cls}-\${s.section})</option>\`).join('');
...
async function refreshGradeTable() {
  ...
  document.getElementById('grd-tbody').innerHTML = grades.map(g => {
    const st = allStudents.find(s => s._id === g.sid);
    const pct = Math.round(g.obt / g.max * 100);
    const col = gradeColor(pct);
    return \`<tr><td><strong>\${st?st.name:'Unknown'}</strong></td><td>\${g.sub}</td><td>\${g.exam}</td><td>\${g.max}</td><td style="font-family:var(--mono);font-weight:700;color:\${col};">\${g.obt}</td><td style="font-family:var(--mono);font-weight:700;color:\${col};">\${pct}%</td><td><span class="badge" style="background:\${col}1a;color:\${col};">\${gradeLabel(pct)}</span></td><td><button class="btn-del" onclick="delGrade('\${g._id}')">Del</button></td></tr>\`;
  }).join('') || '<tr><td colspan="8" class="tbl-empty">No grade records yet</td></tr>';
}
async function saveGrade() {
  ...
  const r = await POST('/api/grades', { sid, sub, exam, max: parseInt(max), obt: parseInt(obt) });
...
async function refreshAttTable() {
  ...
  document.getElementById('att-tbody').innerHTML = att.map(a => {
    const st = allStudents.find(s => s._id === a.sid);
    const pct = Math.round(a.pre / a.tot * 100);
    const col = pct>=75?'var(--green)':pct>=50?'var(--amber)':'var(--rose)';
    return \`<tr><td><strong>\${st?st.name:'Unknown'}</strong></td><td>\${a.sub}</td><td>\${a.tot}</td><td>\${a.pre}</td><td>\${a.tot-a.pre}</td><td style="font-family:var(--mono);font-weight:700;color:\${col};">\${pct}%</td><td><span class="badge \${pct>=75?'b-green':pct>=50?'b-amber':'b-rose'}">\${pct>=75?'Good':pct>=50?'Average':'Warning'}</span></td><td><button class="btn-del" onclick="delAtt('\${a._id}')">Del</button></td></tr>\`;
  }).join('') || '<tr><td colspan="8" class="tbl-empty">No attendance records yet</td></tr>';
}
async function saveAtt() {
  ...
  const r = await POST('/api/attendance', { sid, sub, tot: parseInt(tot), pre: parseInt(pre) });
...
async function refreshHWTable() {
  ...
  document.getElementById('hw-tbody').innerHTML = hw.map(h => {
    ...
    return \`<tr><td><strong>\${h.title}</strong></td><td>\${h.sub}</td><td><span class="badge b-primary">\${h.cls}</span></td><td style="font-family:var(--mono);font-size:12px;">\${h.due}</td><td><span class="badge \${pb}">\${h.pri}</span></td><td><button class="btn-del" onclick="delHW('\${h._id}')">Del</button></td></tr>\`;
...
async function saveHW() {
  ...
  const sub   = document.getElementById('hf-sub').value.trim();
  ...
  document.getElementById('nlist-admin').innerHTML = notices.map((n, i) => \`
    <div class="card" style="animation:pageIn .3s ease \${i*0.04}s both;">
      <div style="display:flex;align-items:center;gap:10px;margin-bottom:8px;">
        ...
        <button class="btn-del" onclick="delNotice('\${n._id}')">Delete</button>
      </div>
...
async function refreshTTTable() {
  ...
  document.getElementById('tt-tbody').innerHTML = tt.map(t =>
    \`<tr><td><span class="badge b-primary">\${t.cls}</span></td><td>\${t.day}</td><td style="font-size:12px;color:var(--text3);">\${t.period}</td><td><strong>\${t.sub}</strong></td><td><button class="btn-del" onclick="delTT('\${t._id}')">Del</button></td></tr>\`
...
async function loadStudentTT() {
  ...
  el.innerHTML = days.map(day => {
    ...
    return \`<div class="card" style="margin-bottom:12px;"><div class="class-section-header">\${day}</div><div style="display:flex;gap:10px;flex-wrap:wrap;">\${entries.map(e => \`<div style="background:var(--bg3);border:1px solid var(--border);border-radius:9px;padding:10px 14px;min-width:120px;"><div style="font-weight:700;font-size:13px;">\${e.sub}</div><div style="font-size:11px;color:var(--text3);margin-top:2px;">\${e.period}</div></div>\`).join('')}</div></div>\`;
  }).join('');
}
